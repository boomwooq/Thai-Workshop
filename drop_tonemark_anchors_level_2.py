...
def get_saraii_top(f):
    if 'th-saraii' in f:
        g = f['th-saraii']
        top = int(round(g.bounds[3]))
        return top
    else:
        print(f'no th-saraii found, using bor height {BOR_HEIGHT}')
        return BOR_HEIGHT
...
if __name__ == '__main__':
    ...
    tonemarks_level_2 = [gn for gn in tonemarks if '.small' in gn]
    glyphs = get_glyphs()
    for g in glyphs:
        if g.name in tonemarks_level_2:
            print(f'✅ dropping anchors in {g.name}')
            rm = most_likely_right_margin(g)
            saraii_top = get_saraii_top(f)
            tonemark_top = g.bounds[3]

            # drop _top anchor
            x_offset = italic_x_offset(saraii_top, f.info.italicAngle)
            _top_anchor_coords = int(round(rm + x_offset)), saraii_top
            ...
            print(f'⛔️ {g.name} not a level 2 tone mark')
